...
import "./ctmgt-modal.scss";
import { useModalContext } from "../../../../context/modal_context";

const CtMgtModal = (props) => {
  const { createNewAccountModalIsHidden, showCreateAccountModalHandler } =
    useModalContext();
  return (
    <div
      className={`pst-absolute h-100vh w-100vw  ${
        createNewAccountModalIsHidden
          ? "content-management-modal show--modal"
          : "content-management-modal hide--modal"
      }`}
    >
      <div className="modal-overlay">
        <div className="modal-card">
          <div
            className={`${
              createNewAccountModalIsHidden
                ? "modal-card__body show--body"
                : "modal-card__body hide--body"
            }`}
          >
            <div className="btn-close flex-container  p-8">
              <button
                onClick={showCreateAccountModalHandler}
                className="mr-lft-auto flex-container j-ct-ctr alg-itms-ctr"
              >
                <svg
                  width="20"
                  height="20"
            ...
            <div className=" h-100 p-5">{props.children}</div>
          </div>
        </div>
      </div>
    </div>
